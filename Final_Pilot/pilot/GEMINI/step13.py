...
# CONFIG
# =========================
INPUT_RAW_JSONL = "data/gemini_raw_outputs.jsonl"
INPUT_PAYLOADS_JSONL = "data/gemini_batch_payloads.jsonl"
INPUT_CONSOLIDATED_XLSX = "data/postConsolidated.xlsx"

OUTPUT_PARSED_XLSX = "data/gemini_parsed_outputs.xlsx"
OUTPUT_PARSED_CSV = "data/gemini_parsed_outputs.csv"
OUTPUT_PARSE_ERRORS_XLSX = "data/gemini_parse_errors.xlsx"

OUTPUT_FINAL_WORKING_XLSX = "data/final_llm_coding_working.xlsx"
OUTPUT_MERGE_ISSUES_XLSX = "data/final_llm_merge_issues.xlsx"

OUTPUT_CONSERVATIVE_XLSX = "data/final_conservative_adjudicated_table.xlsx"
OUTPUT_CONSERVATIVE_CSV = "data/final_conservative_adjudicated_table.csv"

OUTPUT_LLM_AUTH_XLSX = "data/final_llm_authoritative_table.xlsx"
OUTPUT_LLM_AUTH_CSV = "data/final_llm_authoritative_table.csv"

OUTPUT_MANUAL_VERIFY_XLSX = "data/final_manual_verification_table.xlsx"

OUTPUT_HIGH_CONF_XLSX = "data/final_high_confidence_coding_table.xlsx"
OUTPUT_HIGH_CONF_CSV = "data/final_high_confidence_coding_table.csv"
...
def main():
    ...
    # Duplicate raw-output check
    if not df_raw.empty and "article_id" in df_raw.columns:
    ...
    # Robust replacement for fragile groupby(...).apply(...)
    best_rows = []
    if not df_raw.empty and "article_id" in df_raw.columns:
    ...
    # Merge issues
    payload_ids = set(df_payload_sub["Article_ID"].dropna().tolist()) if "Article_ID" in df_payload_sub.columns else set()
    parsed_ids = set(df_parsed["Article_ID"].dropna().tolist()) if "Article_ID" in df_parsed.columns else set()
    raw_ids = set(df_raw_best["article_id"].dropna().tolist()) if "article_id" in df_raw_best.columns else set()
    cons_ids = set(df_cons["Article_ID"].dropna().tolist()) if "Article_ID" in df_cons.columns else set()
    ...
    for aid in sorted(invalid_or_missing_ids):
        payload_match = df_payload[df_payload["article_id"] == aid]
        raw_match = df_raw_best[df_raw_best["article_id"] == aid] if "article_id" in df_raw_best.columns else pd.DataFrame()
        payload_meta = payload_match.iloc[-1].to_dict() if not payload_match.empty else {}
