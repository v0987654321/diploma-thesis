...
def main():
    ...
    # =========================
    # ARTICLE LAYER: CONSERVATIVE GEMINI ADJUDICATION
    # =========================
    # The original thesis workflow copied the conservative Gemini table into
    # CORPUS/data but did not merge its Adj_V* coding values into review_master.
    # This article-specific workflow explicitly merges those values, creates
    # Final_* variables, preserves original pipeline codes as Pipeline_* fields,
    # and replaces operational coding columns used by downstream article scripts.

    if gemini_conservative is None:
        raise FileNotFoundError(
            "Article workflow requires Gemini conservative adjudicated output, "
            "but final_conservative_adjudicated_table.csv was not found."
        )
    ...
    article_adj_keep = [
        col for col in article_adj_keep
        if col in gemini_conservative.columns
    ]

    review_master = review_master.merge(
        gemini_conservative[article_adj_keep].drop_duplicates(
            subset=["Article_ID"],
            keep="last"
        ),
        on="Article_ID",
        how="left",
        suffixes=("", "_article_duplicate")
    )
    ...
    def article_coalesce_code(adjudicated_value, pipeline_value):
        if pd.notna(adjudicated_value):
            try:
                return int(float(adjudicated_value))
            except Exception:
                return adjudicated_value

        if pd.notna(pipeline_value):
            try:
                return int(float(pipeline_value))
            except Exception:
                return pipeline_value

        return None

    for final_col, (adj_col, pipeline_col) in article_final_map.items():
        if pipeline_col not in review_master.columns:
            review_master[pipeline_col] = None

        if adj_col not in review_master.columns:
            review_master[adj_col] = None

        # Preserve thesis/pipeline value for audit.
        review_master[f"Pipeline_{pipeline_col}"] = review_master[pipeline_col]

        review_master[final_col] = review_master.apply(
            lambda row: article_coalesce_code(
                row.get(adj_col),
                row.get(pipeline_col)
            ),
            axis=1
        )

        # Article workflow operational coding is final/adjudicated where available.
        review_master[pipeline_col] = review_master[final_col]

    review_master["Article_Coding_Layer"] = "Gemini conservative adjudication with pipeline fallback"
    ...
    review_master["chapter52_priority_flag"] = (
        (
            ...
            (review_master.get("Final_Human_Rights_Abuse", pd.Series(0, index=review_master.index)).fillna(0).astype(int) == 1) |
            (review_master.get("Final_Security_Effectiveness", pd.Series(0, index=review_master.index)).fillna(0).astype(int) == 1) |
            (review_master.get("Final_Dominant_Discourse", pd.Series(0, index=review_master.index)).fillna(0).astype(int).isin([2, 3, 4])) |
