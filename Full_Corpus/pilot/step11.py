...
# =========================
def main():
    print("Loading step files for diagnostic table...")
    ...
    # 3. BASE FROM STEP 1
    # =========================
    print("Preparing Step1 diagnostic base...")
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        print_progress(i, total, label="Review meta progress", every=200)
    ...
    print(f"Relevance 1: {(df_out['Relevance'] == 1).sum() if 'Relevance' in df_out.columns else 'NA'}")
    print(f"Relevance 2: {(df_out['Relevance'] == 2).sum() if 'Relevance' in df_out.columns else 'NA'}")
    print(f"Relevance 3: {(df_out['Relevance'] == 3).sum() if 'Relevance' in df_out.columns else 'NA'}")
    print(f"Relevance 4: {(df_out['Relevance'] == 4).sum() if 'Relevance' in df_out.columns else 'NA'}")
